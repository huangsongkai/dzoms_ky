...
public class DriverAction extends BaseAction {
    private static final DualHashBidiMap photoNameMap = new DualHashBidiMap();
...
//			}
            ...
            } else if (driver.getStatus() == 2 && isCaiWuJinLi) {
...
//			if(file.exists()&&file.delete()){
    ...
    public String driverAdd() throws IOException {
        ...
        if (StringUtils.length(P_IdCard) == 30)
            FileUploadUtil.store(P_IdCard, new File(baseFile, "P_IdCard.jpg"));
        if (StringUtils.length(P_DriverLicense) == 30)
            FileUploadUtil.store(P_DriverLicense, new File(baseFile, "P_DriverLicense.jpg"));
        if (StringUtils.length(P_Qualification) == 30)
            FileUploadUtil.store(P_Qualification, new File(baseFile, "P_Qualification.jpg"));
        if (StringUtils.length(P_Location) == 30)
            FileUploadUtil.store(P_Location, new File(baseFile, "P_Location.jpg"));
        if (StringUtils.length(P_TempLocton) == 30)
            FileUploadUtil.store(P_TempLocton, new File(baseFile, "P_TempLocton.jpg"));
        if (StringUtils.length(P_EmployContract) == 30)
            FileUploadUtil.store(P_EmployContract, new File(baseFile, "P_EmployContract.jpg"));
        if (StringUtils.length(P_Server) == 30)
            FileUploadUtil.store(P_Server, new File(baseFile, "P_Server.jpg"));
        if (StringUtils.length(P_photo) == 30)
            FileUploadUtil.store(P_photo, new File(baseFile, "P_photo.jpg"));
        if (StringUtils.length(P_vehicle) == 30)
            FileUploadUtil.store(P_vehicle, new File(baseFile, "P_vehicle.jpg"));
        if (StringUtils.length(P_table) == 30)
            FileUploadUtil.store(P_table, new File(baseFile, "P_table.jpg"));
        if (StringUtils.length(P_personal) == 30)
            FileUploadUtil.store(P_personal, new File(baseFile, "P_personal.jpg"));
        if (StringUtils.length(P_praise) == 30)
            FileUploadUtil.store(P_praise, new File(baseFile, "P_praise.jpg"));
